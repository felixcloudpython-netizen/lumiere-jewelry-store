  ...
  return (
    <main className="pt-28 pb-20">
      <div className="max-w-[1400px] mx-auto px-6">
        ...
        {categories.length > 0 && (
          <div className="mb-14">
            <p className="text-[11px] tracking-[0.2em] uppercase text-neutral-400 text-center mb-6">{t('shopByCategory')}</p>
            <div className="flex flex-wrap items-center justify-center gap-3">
              {categories.map((cat) => (
                <Link
                  key={cat.id}
                  href={`/${locale}/jewelry/${cat.slug}`}
                  className="px-6 py-3 border border-neutral-200 text-xs tracking-[0.15em] uppercase hover:border-neutral-900 hover:bg-neutral-900 hover:text-white transition-colors"
                >
